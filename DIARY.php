<!doctype html>
<html>
<head>

	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1, user-scalable=no">
	<link rel="stylesheet" href="CSS.css">
	<title>DIARY</title>
	
</head>
<body id="BG">
	
	<hr color="#FF6E00" size="6">
	<hr color="#FFB600" size="6">
	
	<center>
		<img src="Images/DCinemas-02.jpg" width="100%" id="DCLOGO" style="box-shadow: 10px 2px 12px #000000">
		<img src="Images/DCinemas-2_Mob.jpg" id="DCLOGO2" style="box-shadow: 10px 2px 12px #000000">
	</center>
	
	<br>
	<table align="center">
		<tr>
			<td>
				<a href="index.php">
					<button class="navigationBtn"><font face="Britamic" size="+1" color="#FFFFFF" class="Shadow">HOME</font></button>
				</a>
			</td>
			
			<td>
				<div class="dropdown">
					<button class="navigationBtn">
						<font face="Britamic" size="+1" color="#FFFFFF" class="Shadow">CREATIONS</font>
					</button>
					
					<div class="dropdown-content">
						<a href="Films.php">
							<button class="SUBbtn; Shadow" id="CREATIONS1">
								<font face="Britamic" color="#FFFFFF" class="Shadow">
									FILMS
								</font>
							</button>	
						</a>

						<a href="Shortfilms.php">
							<button class="SUBbtn; Shadow" id="CREATIONS2">
								<font face="Britamic" color="#FFFFFF" class="Shadow">
									SHORTFILMS
								</font>
							</button>
						</a>

						<a href="Thedal.php">
							<button class="SUBbtn; Shadow" id="CREATIONS3">
								<font face="Britamic" color="#FFFFFF" class="Shadow">
									THEDAL
								</font>
							</button>
						</a>
					</div>
				</div>
			</td>
			
			<td>
				<div class="dropdown">
					<button class="navigationBtn">
						<font face="Britamic" size="+1" color="#FFFFFF" class="Shadow">GROUP</font>
					</button>

					<div class="dropdown-content">
						<a href="Members.php">
							<button class="SUBbtn; Shadow" id="MEMBERS1">
								<font face="Britamic" color="#FFFFFF" class="Shadow">
									MEMBERS
								</font>
							</button>
						</a>

						<a href="Self_Works.php">
							<button class="SUBbtn; Shadow" id="MEMBERS2">
								<font face="Britamic" color="#FFFFFF" class="Shadow">
									WORKS
								</font>
							</button>
						</a>
					</div>
				</div>
			</td>
			
			<td>
				<a href="More.php">
					<button class="navigationBtn">
						<font face="Britamic" size="+1" color="#FFFFFF" class="Shadow">
							MORE
						</font>
					</button>
				</a>
			</td>
		</tr>
	</table>
	
	<table align="center" border="0" cellspacing="40px" id="M">
		<tr>
			<td align="center">
				<p>
					<h1>
						<font face="Adobe Caslon Pro" size="+4" style="text-shadow: 1px 1px 10px black; color:orange">
							<i>-- 01. DIARY THE FIND OUT (2019) --</i>
						</font>
					</h1>

					<small><font face="Adobe Caslon Pro" color="#FFFFFF" style="text-shadow: 1px 1px 5px black">
						18 MIN 35 SEC | CASUAL | PRACTISE
						<br>RATE : ___&emsp;&ensp;MAX.POINTS : ___&emsp;&ensp;MIN.POINTS : ___

					</font></small>

					<br><br><br>
					<img src="Images/DIARY2.jpg" height="300" width="550" id="YAAR" style="box-shadow: 10px 2px 12px #000000">

					<br>
					<font face="Adobe Caslon Pro" size="+2" color="#FFFFFF" style="text-shadow: 1px 1px 12px black">
						<br>It's A Short Film Producing By DRAGON CINEMAS. This Is Our First Creation And A Trial Creation For All Our Future
						Creations. This Short Film Was A Practise For Us. So There Was Nothing Big Story And A Message.This Is Casual Story With
						A Hero, Sub Hero &amp; A Normal Villan. A Student Studying In Students Science Club, OneDay He Found A Diary Inside with
						A Treasure Map. He don't Know About Anything About That Diary. That's True Owner Searching That, OnTheWay He
						Founds SomeOne Gets It From The Place It's Missed. And Also The Villan Was Searching That Diary For The Diamond.
						Diary's Owner &amp; The Villan Founds Him. After That, Story Was Moving With The Diamond And Who Gets It Lastly.

						<br><br>OFFICIAL TRAILER : 
						<a href="http://youtu.be/b9phCjKOd3g" class="aLink">http://youtu.be/b9phCjKOd3g</a>
						<br>FULL SHORT FILM : 
						<a href="http://youtu.be/mlCRLZIqekI" class="aLink">http://youtu.be/mlCRLZIqekI</a>

						<br><br>
						<form method="get" onClick="window.open('Scripts/DiaryScript.pdf')">
							<button type="submit">
								<font size="+2" style="text-shadow: 1px 1px 5px brown" class="aLink">SCRIPT IS AVAILABLE NOW</font>
							</button>
						</form>

						<br><br>Writters : 
						<a href="Members.php#G" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#SR" class="aLink">SAM ROHAN</a>
						<br>Directors : 
						<a href="Members.php#M" class="aLink">MADUSHANTH</a> ,
						<a href="Members.php#G" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#SS" class="aLink">SRI SATHIYAN</a>
						<br>Producers : 
						<a href="Members.php#SS" class="aLink">SRI SATHIYAN</a> ,
						<a href="Members.php#G" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#M" class="aLink">MADUSHANTH</a>
						<br>Editor : 
						<a href="Members.php#M" class="aLink">MADUSHANTH (M CREATIONS)</a>
						<br>Cinematography : 
						<a href="Members.php#M" class="aLink">MADUSHANTH</a>
						<br>Stunts : 
						<a href="Members.php#R" class="aLink">ROHIT</a> ,
						<a href="Members.php#M" class="aLink">MADUSHANTH</a>
						<br>Stars : 
						<a href="Members.php#D" class="aLink">DHANUSH</a> ,
						<a href="Members.php#SR" class="aLink">SAM ROHAN</a> ,
						<a href="Members.php#R" class="aLink">ROHIT</a> ,
						<a href="Members.php#G" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#JJ" class="aLink">JOHNSAN JEEWA</a> ,
						<a href="Members.php#SS" class="aLink">SRI SATHIYAN</a> ,
						<a class="aLink">GOWTHAMAN</a>
					</font>
				</p>
			</td>
		</tr>
	</table>
	<table align="center" border="0" cellspacing="10" id="M2">
		<tr>
			<td align="center">
				<p>
					<h2>
						<font face="Adobe Caslon Pro" size="+2" style="text-shadow: 1px 1px 10px black; color:orange">
							<i>-- 01. <br>DIARY THE FIND OUT (2019) --</i>
						</font>
					</h2>

					<small><font face="Adobe Caslon Pro" color="#FFFFFF" style="text-shadow: 1px 1px 5px black">
						18 MIN 35 SEC | CASUAL | PRACTISE
						<br>RATE : ___ MAX.POINTS : ___ MIN.POINTS : ___

					</font></small>

					<br><br><br>
					<img src="Images/DIARY2.jpg" height="300" width="550" id="DIARY" style="box-shadow: 10px 2px 12px #000000">

					<br>
					<font face="Adobe Caslon Pro" size="+1" color="#FFFFFF" style="text-shadow: 1px 1px 12px black">
						<br>It's A Short Film Producing By DRAGON CINEMAS. This Is Our First Creation And A Trial Creation For All Our Future
						Creations. This Short Film Was A Practise For Us. So There Was Nothing Big Story And A Message.This Is Casual Story With
						A Hero, Sub Hero &amp; A Normal Villan. A Student Studying In Students Science Club, OneDay He Found A Diary Inside with
						A Treasure Map. He don't Know About Anything About That Diary. That's True Owner Searching That, OnTheWay He
						Founds SomeOne Gets It From The Place It's Missed. And Also The Villan Was Searching That Diary For The Diamond.
						Diary's Owner &amp; The Villan Founds Him. After That, Story Was Moving With The Diamond And Who Gets It Lastly.

						<br><br>OFFICIAL TRAILER : 
						<a href="http://youtu.be/b9phCjKOd3g" class="aLink">http://youtu.be/b9phCjKOd3g</a>
						<br>FULL SHORT FILM : 
						<a href="http://youtu.be/mlCRLZIqekI" class="aLink">http://youtu.be/mlCRLZIqekI</a>

						<br><br>
						<form method="get" onClick="window.open('Scripts/DiaryScript.pdf')">
							<button type="submit">
								<font size="+2" style="text-shadow: 1px 1px 5px brown" class="aLink">SCRIPT IS AVAILABLE NOW</font>
							</button>
						</form>

						<br><br>Writters : 
						<a href="Members.php#G2" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#SR2" class="aLink">SAM ROHAN</a>
						<br>Directors : 
						<a href="Members.php#M2" class="aLink">MADUSHANTH</a> ,
						<a href="Members.php#G2" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#SS2" class="aLink">SRI SATHIYAN</a>
						<br>Producers : 
						<a href="Members.php#SS2" class="aLink">SRI SATHIYAN</a> ,
						<a href="Members.php#G2" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#M2" class="aLink">MADUSHANTH</a>
						<br>Editor : 
						<a href="Members.php#M2" class="aLink">MADUSHANTH (M CREATIONS)</a>
						<br>Cinematography : 
						<a href="Members.php#M2" class="aLink">MADUSHANTH</a>
						<br>Stunts : 
						<a href="Members.php#R2" class="aLink">ROHIT</a> ,
						<a href="Members.php#M2" class="aLink">MADUSHANTH</a>
						<br>Stars : 
						<a href="Members.php#D2" class="aLink">DHANUSH</a> ,
						<a href="Members.php#SR2" class="aLink">SAM ROHAN</a> ,
						<a href="Members.php#R2" class="aLink">ROHIT</a> ,
						<a href="Members.php#G2" class="aLink">GOWSALJAN</a> ,
						<a href="Members.php#JJ2" class="aLink">JOHNSAN JEEWA</a> ,
						<a href="Members.php#SS2" class="aLink">SRI SATHIYAN</a> ,
						<a class="aLink">GOWTHAMAN</a>
					</font>
				</p>
			</td>
		</tr>
	</table>
	
	<br><br>
	<hr color="#FFB600" size="6">
	<hr color="#FF6E00" size="6">
</body>
</html>